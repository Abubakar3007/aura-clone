    ...
    <section className="py-20 bg-secondary">
      <div className="max-w-[1200px] w-full mx-auto xl:px-0 px-4">
        <div>
          ...
          <div className="grid md:grid-cols-3 sm:grid-cols-2 gap-6 mb-8">
            {
              featuredImages.map((image, index) => (
                <div key={index} className="relative overflow-hidden aspect-[3/4] rounded-[4px]">
                  <img
                    src={image.src}
                    alt={image.title}
          ...
          <Link 
            href="/auraaedge" 
            title="View model profile"
            className="btn-primary armata mx-auto w-fit flex h-[60px]"
          >
            View Profile
          </Link>
        </div>
